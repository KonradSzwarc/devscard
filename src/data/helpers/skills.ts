import type { LevelledSkill, Skill } from '@/types/sections/skills-section.types';

interface SkillFactory {
  (data: Partial<Skill> & { level: LevelledSkill['level'] }): LevelledSkill;
  (data?: Partial<Skill>): Skill;
}

const createSkill = (defaultData: Omit<Skill, 'description'>) =>
  ((data?: Partial<Skill> & { level?: LevelledSkill['level'] }): Skill | LevelledSkill => ({
    ...defaultData,
    ...data,
  })) as SkillFactory;
