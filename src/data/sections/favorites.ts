import type { FavoritesSection } from '@/types/favorites-section';

const favoritesData: FavoritesSection = {
  config: {
    title: 'My favorites',
    icon: 'fa6-solid:star',
  },
  books: {
    title: 'Books I read',
    data: [
      {
        cover: 'https://m.media-amazon.com/images/I/51-G-4SUzpL._SX258_BO1,204,203,200_QL70_ML2_.jpg',
        title: 'The Pragmatic Programmer: From Journeyman to Master',
        author: 'Andy Hunt, Dave Thomas',
        url: 'https://www.goodreads.com/book/show/4099.The_Pragmatic_Programmer',
      },
      {
        cover: '/assets/favorites/books/book-2.jpeg',
        title: 'Domain-Driven Design: Tackling Complexity in the Heart of Software',
        author: 'Eric Evans',
        url: 'https://www.goodreads.com/book/show/179133.Domain_Driven_Design',
      },
      {
        cover: '/assets/favorites/books/book-3.jpeg',
        title: 'Clean Code: A Handbook of Agile Software Craftsmanship',
        author: 'Robert C. Martin',
        url: 'https://www.goodreads.com/book/show/3735293-clean-code',
      },
      {
        cover: '/assets/favorites/books/book-4.jpeg',
        title: 'The Clean Coder: A Code of Conduct for Professional Programmers',
        author: 'Robert C. Martin',
        url: 'https://www.goodreads.com/book/show/10284614-the-clean-coder',
      },
    ],
  },
  people: {
    title: 'People I learn from',
    data: [
      {
        image: '/assets/favorites/people/person-1.jpg',
        name: 'Kent C. Dodds',
        url: 'https://kentcdodds.com/',
      },
      {
        image: '/assets/favorites/people/person-2.jpeg',
        name: 'Kent Beck',
        url: 'https://www.kentbeck.com/',
      },
      {
        image: '/assets/favorites/people/person-3.jpeg',
        name: 'Eric Evans',
        url: 'https://www.domainlanguage.com/',
      },
      {
        image: '/assets/favorites/people/person-4.jpeg',
        name: 'Martin Fowler',
        url: 'https://martinfowler.com/',
      },
      {
        image: '/assets/favorites/people/person-5.jpg',
        name: 'Robert C. Martin',
        url: 'http://cleancoder.com/',
      },
      {
        image: '/assets/favorites/people/person-6.jpeg',
        name: 'Adam Dymitruk',
        url: 'https://eventmodeling.org/',
      },
    ],
  },
  videos: {
    title: 'Videos I watched',
    data: [
      {
        thumbnail: '/assets/favorites/videos/video-1.jpeg',
        title: 'Building Resilient Frontend Architecture • Monica Lent • GOTO 2019',
        url: 'https://youtu.be/TqfbAXCCVwE',
      },
      {
        thumbnail: '/assets/favorites/videos/video-2.jpeg',
        title: 'Scaling Yourself • Scott Hanselman • GOTO 2012',
        url: 'https://youtu.be/FS1mnISoG7U',
      },
      {
        thumbnail: '/assets/favorites/videos/video-3.jpeg',
        title: "Why Isn't Functional Programming the Norm? - Richard Feldman",
        url: 'https://youtu.be/QyJZzq0v7Z4',
      },
    ],
  },
  medias: {
    title: 'Media I follow',
    data: [
      {
        image: '/assets/favorites/media/media-1.jpeg',
        title: 'Fireship.io',
        type: 'YouTube channel',
        url: 'https://www.youtube.com/c/Fireship',
      },
      {
        image: '/assets/favorites/media/media-2.jpeg',
        title: 'Healthy Software Developer',
        type: 'YouTube channel',
        url: 'https://www.youtube.com/channel/UCfe_znKY1ukrqlGActlFmaQ',
      },
      {
        image: '/assets/favorites/media/media-3.png',
        title: 'Bytes',
        type: 'Newsletter',
        url: 'https://bytes.dev/',
      },
      {
        image: '/assets/favorites/media/media-4.png',
        title: 'TypeScript Weekly',
        type: 'Newsletter',
        url: 'https://typescript-weekly.com/',
      },
      {
        image: '/assets/favorites/media/media-5.jpeg',
        title: 'Front End Happy Hour',
        type: 'Podcast',
        url: 'https://www.frontendhappyhour.com/',
      },
      {
        image: '/assets/favorites/media/media-6.webp',
        title: '.cult by Honeypot',
        type: 'Blog',
        url: 'https://cult.honeypot.io/',
      },
    ],
  },
};

export default favoritesData;
